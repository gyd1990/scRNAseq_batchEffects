...
The genewise proportion of zeros is calculated.

```{r}
propZeros <- data[, .(z = sum(count == 0) / length(count)), by = gene]$z
propZeros <- propZeros[!doNotUse]
summary(propZeros)
stats$propZeros <- propZeros
Hist(stats$propZeros, "proportion of zeros")
```

Proportion of zero counts per gene is known to be dependent on average expression.
Zeros were excluded in gene average expression calculation.

```{r}
ScatterDensity(stats$aveExp, stats$propZeros, "average expression", "proportion of zeros", ld = "x",
               title = "Genewise Proportion of 0's over Average Expression")
```

### Model Zero Proportions
...
A Hill equation is fitted using non-linear least squares.
To make the model more robust outliers are excluded.

```{r}
dt <- stats[, .(z = propZeros, a = aveExp)]
dt <- dt[a > quantile(a, 0.05) & a < quantile(a, 0.95), ]
fit <- nls(
  z ~ 1 - a^n / (Km + a^n), 
  data = dt, 
  start = list(Km = median(dt$a), n = 1.5),
  control = list(maxiter = 100, warnOnly = TRUE)
)
allModels$batch[[btchCount]]$zeros <- fit
summary(fit)
PlotNLS(fit, dt$z)
s <- seq(min(stats$ld_aveExp), max(stats$ld_aveExp), length.out = 1000)
PlotPredict(
  stats$ld_aveExp, stats$propZeros,
  newdata = s,
  prediction = predict(fit, newdata = list(a = 2^s)),
  "log2 average expression", "proportion of zeros",
  "Proportion of 0's over Average Expression Model"
)
```
