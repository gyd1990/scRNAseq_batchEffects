#!/usr/bin/env Rscript
#
# simulate 3 scRNA-seq datasets
# here some parameters are varied and given in "params.csv"
# simulations are saved in "./simX/data/" w/ X = {1, 2, 3}
# directories must already exist
#
# K, A, B are average log2 transformed parameters for BetaPois
# their coefficients of variation are suffixed with CV
# (CV: within a batch, CV2: between batches)
#
# Km and n are used in a Hill function to adjust for 0s (dropouts)
# this adjusted dropoutrate has a std of PZero_SD
#
# Average number of cells per batch is given with NClls
# their coefficient of variation is nClls_CV
# NGns gives number of genes in each batch
#
...
#' PoiBeta
#'
#' Create Poisson-Beta distribution.
#'
...
#'
#' The defaults roughly resemble the \emph{Tung} 2016 dataset.
...
ds <- Generator(
  k = K, k_CV = K_CV, k_CV2 = K_CV2,
  a = A, a_CV = A_CV, a_CV2 = A_CV2,
  b = B, b_CV = B_CV, b_CV2 = B_CV2,
  Km = KM, n = N,
  pZero_SD = PZero_SD,
  nBatches = NBtchs,
  nGenes = NGns,
...
saveRDS(ds, "sim1/data/simulation.rds")
...
saveRDS(ds, "sim2/data/simulation.rds")
...
saveRDS(ds, "sim3/data/simulation.rds")
